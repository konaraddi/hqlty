...
- Use autogenerated sidebar ordered by `sidebar_position` (mapping from current priority system)
...
priority: high          # optional, defaults to "medium" via p.11tydata.json
---
```

The `layout` and `tags` fields are injected by `p.11tydata.json` and are 11ty-specific.

### Priority distribution (for sidebar_position mapping)
Patterns with explicit `priority: high` (5 patterns):
- downvote-martyrdom
- hindsight-dismissal
...
Patterns with explicit `priority: low` (3 patterns):
- history-lesson
- purity-gatekeeping
- performative-defeatism

All other patterns (21 patterns): default `medium` priority.
...
---
title: Hindsight Dismissal
sidebar_position: 10         # high=10-59, medium=100-159, low=200-259
---
```
...
- `priority` is replaced by `sidebar_position` for ordering
- `layout` and `tags` are removed (11ty-specific)

### sidebar_position mapping strategy

Use ranges to preserve priority ordering while allowing alphabetical sort within tiers:
- **high priority**: positions 10, 11, 12, 13, 14 (alphabetical within tier)
- **medium priority**: positions 100, 101, 102, ... (alphabetical within tier)
- **low priority**: positions 200, 201, 202, ... (alphabetical within tier)

This leaves gaps for easy insertion of new patterns without renumbering.
...
1. For each of the 29 pattern `.md` files in `src/p/`:
   ...
   - Remove `priority` and replace with `sidebar_position` using the mapping:
     - high priority patterns (alphabetically sorted, positions 10+):
       - downvote-martyrdom.md -> 10
       - hindsight-dismissal.md -> 11
       - semantic-derailment.md -> 12
       - strawman-disclaimer.md -> 13
       - victory-lap-agreement.md -> 14
     - medium priority patterns (alphabetically sorted, positions 100+):
       - achievement-minimization.md -> 100
       - attention-policing.md -> 101
       - condescending-reveal.md -> 102
       - diy-dismissal.md -> 103
       - elitist-dismissal.md -> 104
       - fallacy-labeling.md -> 105
       - literal-interpretation.md -> 106
       - meta-grievance-deflection.md -> 107
       - motivation-diagnosis.md -> 108
       - opportunistic-pivot.md -> 109
       - pedantic-deflection.md -> 110
       - precision-deflection.md -> 111
       - preemptive-condescension.md -> 112
       - preemptive-martyrdom.md -> 113
       - research-dismissal.md -> 114
       - sci-fi-dismissal.md -> 115
       - sinister-analogy.md -> 116
       - source-dismissal.md -> 117
       - strawman-mockery.md -> 118
       - vague-they.md -> 119
     - low priority patterns (alphabetically sorted, positions 200+):
       - history-lesson.md -> 200
       - performative-defeatism.md -> 201
       - purity-gatekeeping.md -> 202
...
- [ ] Sidebar order matches priority tiers (high first, then medium, then low)
...
2. Fill in frontmatter: `title` and `sidebar_position` (pick a number in the appropriate range)
